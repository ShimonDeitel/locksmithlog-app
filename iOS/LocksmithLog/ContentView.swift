...
struct ContentView: View {
    ...
    @State private var editingEntry: VisitEntry?
...
struct EntryFormView: View {
    ...
    init(mode: EntryFormMode) {
        ...
        if case .edit(let entry) = mode {
            ...
            _doorName = State(initialValue: entry.doorName)
    ...
    var body: some View {
        NavigationStack {
            Form {
                DatePicker("Service Date", selection: $date, displayedComponents: .date)
                    .accessibilityIdentifier("entryDateField")
                TextField("Door", text: $doorName)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    ...
                    .disabled(doorName.trimmingCharacters(in: .whitespaces).isEmpty)
                }
            }
        }
    }
    ...
    private func save() {
        switch mode {
        case .add:
            let entry = VisitEntry(date: date, doorName: doorName, notes: notes)
            store.add(entry)
        case .edit(let existing):
            var updated = existing
            updated.date = date
            updated.doorName = doorName
            updated.notes = notes
